...
const rooms = [
  ...
  { id: "community", name: "Student Community", icon: "👨‍🎓" },
...
const Chat = () => {
  ...
  return (
    <div className="flex h-[calc(100vh-5rem)] overflow-hidden border-t border-[#c5e3f3]">
      {/* Sidebar - Hidden on mobile, visible on md screens */}
      <div className="hidden md:block w-64 bg-[#d9eef9] text-[#173b57] p-4 overflow-y-auto">
            ...
            <button
              ...
              className={`w-full text-left px-4 py-2 rounded-lg flex items-center space-x-2 transition-colors ${
                selectedRoom === room.id
                  ? "bg-[#77beea] text-[#173b57] shadow-sm"
                  : "hover:bg-[#c5e3f3]"
              }`}
            >
      ...
      <div className="flex-1 flex flex-col bg-gray-100 overflow-hidden">
        {/* Room Header */}
        <div className="bg-[#d9eef9] p-4 border-b border-[#c5e3f3] shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              ...
              {!isConnected && !error && (
                <p className="text-[#b7c6c2] text-sm mt-1">Connecting to chat server...</p>
              )}
            </div>
        ...
        {/* Messages */}
        <div ref={messagesListRef} className="flex-1 overflow-y-auto p-4 space-y-4">
          {isLoading ? (
                    ...
                    <div
                      className={`rounded-lg px-4 py-2 ${
                        isAdmin
                          ...
                          : isCurrentUser
                          ? 'bg-[#173b57] text-white rounded-tr-none'
                          : 'bg-white text-[#173b57] rounded-tl-none'
                      }`}
                    >
                      {!isCurrentUser && (
        ...
        {/* Message Input */}
        {user ? (
          <div className="bg-white p-4 border-t border-[#c5e3f3]">
            <form onSubmit={handleSendMessage} className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2 rounded-lg border border-[#c5e3f3] focus:outline-none focus:border-[#1976b9]"
              />
              <button
                type="submit"
                disabled={!isConnected}
                className="px-4 py-2 bg-[#173b57] text-white rounded-lg hover:bg-[#102c43] disabled:opacity-50 disabled:cursor-not-allowed"
              >
          ...
          <div className="p-4 text-center bg-white border-t border-[#c5e3f3]">
            <p className="text-gray-600 mb-2">You are required to login to use this feature.</p>
            <Link to="/login">
              <button className="bg-[#77beea] text-[#173b57] hover:bg-[#5aa9d6] py-2 px-4 rounded-lg font-medium transition-colors">Login</button>
            </Link>
          </div>
        )}
      </div>
